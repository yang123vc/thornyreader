...
void MuPdfBridge::processLinks(int pageNo, CmdResponse& response)
{
    ...
    do
    {
        if (link->dest.kind == FZ_LINK_URI)
        {
            ...
            response.addWords((uint16_t) EBD_URI_LINK, 0);
        ...
        else if (link->dest.kind == FZ_LINK_LAUNCH)
        {
            ...
            DEBUG_L(L_DEBUG_LINKS, LCTX, "Launch link found on page %d: %s", pageNo, link->dest.ld.launch.file_spec);

            char* uri = link->dest.ld.launch.file_spec;
            response.addWords((uint16_t) EBD_LAUNCH_LINK, 0);
            response.addIpcString(uri != NULL ? uri : EMPTY_URL, false);
            response.addFloat(left);
            response.addFloat(top);
            response.addFloat(right);
        ...
        }else if (link->dest.kind == FZ_LINK_GOTO)
        {
            ...
            if (target)
            {
                ...
                response.addWords((uint16_t) EBD_PAGE_LINK, (uint16_t) link->dest.ld.gotor.page);
                response.addFloat(left);
                response.addFloat(top);
                response.addFloat(right);
                response.addFloat(bottom);
        ...
        else if (link->dest.kind == FZ_LINK_GOTOR)
        {
            ...
            if (target)
            {
                ...
                response.addWords((uint16_t) EBD_OUTER_PAGE_LINK, (uint16_t) link->dest.ld.gotor.page);
                response.addFloat(left);
                response.addFloat(top);
                response.addFloat(right);
                response.addFloat(bottom);
