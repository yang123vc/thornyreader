...
fz_draw_fill_image(fz_device *devp, fz_image *image, const fz_matrix *ctm, float alpha)
{
	...
	fz_try(ctx)
	{
		...
		if (ctx->ebookdroid_nightmode) {
			...
			fz_paint_image(state->dest, &state->scissor, state->shape, inverted, &local_ctm, alpha * 255);
		} else {
	...
	fz_colorspace *colorspace, float *color, float alpha)
{
	...
	fz_try(ctx)
	{
		...
		if (ctx->ebookdroid_nightmode) {
			...
			fz_paint_image_with_color(state->dest, &state->scissor, state->shape, inverted, &local_ctm, colorbv);
		} else {
