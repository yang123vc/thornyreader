...
{
	fz_file_stream *state = stm->state;
	/// EBD: corrupted files with garbage on start >>>
	if (whence == SEEK_SET) {
		offset += ctx->ebookdroid_file_stream_offset;
	}
	/// EBD: corrupted files with garbage on start <<<
	...
	/// EBD: corrupted files with garbage on start >>>
	stm->pos = n - ctx->ebookdroid_file_stream_offset;
	/// EBD: corrupted files with garbage on start <<<
	stm->rp = state->buffer;
	stm->wp = state->buffer;
}
...
// EBD: meta implementation for file >>>
