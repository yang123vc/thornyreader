...
void toResponse(CmdResponse& response, fz_rect& bounds, fz_irect* rr, const char* str, int len)
{
    ...
    response.add(right);
    response.add(bottom);
    response.add(utf8, true);
}

void processLine(CmdResponse& response, fz_rect& bounds, fz_text_line& line)
{
    int index = 0;
    ...
    for (span = line.first_span; span != NULL; span = span->next)
    {
        ...
        if (span->text && span->len > 0)
        {
            ...
            for (textIndex = 0; textIndex < span->len;)
            {
                ...
                {
                    fz_rect bbox;
                    fz_text_char_bbox(&bbox, span, textIndex);
    ...
    fz_try(ctx)
            {
                ...
                fz_run_page(document, page, dev, &fz_identity, NULL);
                        ...
                        if (block.u.text->lines && block.u.text->len > 0)
                        {
                            ...
                            for (lineIndex = 0; lineIndex < block.u.text->len; lineIndex++)
                            {
                                ...
                                if (line.first_span)
                                {
                                    ...
                                    processLine(response, bounds, line);
                                }
                            }
                        }
                    }
            ...
            fz_always(ctx)
            {
                ...
                if (pagetext)
                {
                    ...
                    fz_free_text_page(ctx, pagetext);
                }
                if (sheet)
                {
