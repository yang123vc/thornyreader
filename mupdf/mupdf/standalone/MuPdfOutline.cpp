...
{
    response.add((uint32_t) level);
    response.add(title != NULL ? title : EMPTY_STR, false);
    response.add((uint16_t) kind, pageNo);

    if (kind == FZ_LINK_URI)
    {
        response.add(url != NULL ? title : EMPTY_STR, false);
    }
    else if (kind == FZ_LINK_GOTO)
    {
        response.add(x).add(y);
    }
}
