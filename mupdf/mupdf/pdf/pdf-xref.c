...
{
	char buf[20];
	/// EBD: files with garbage in the beginning >>>
	int i;

	for (i = 0; i < 1024; i++)
	{
		ctx->ebookdroid_file_stream_offset = i;
	...
	ctx->ebookdroid_file_stream_offset = 0;
	/// EBD: files with garbage in the beginning <<<
}
...
// EBD: First try to find last %%EOF marker >>>
	int found_eof = 0;
	do
	{
...
pdf_init_document(fz_context *ctx, pdf_document *doc)
{
	...
	{
// EBD: proper reparing >>>
		ctx->ebookdroid_reparing = 0;
// EBD: proper reparing <<<

		pdf_load_version(ctx, doc);
// EBD: files with garbage at beginning >>>
		doc->file_length = fz_stream_meta(ctx, doc->file, FZ_STREAM_META_LENGTH, 0, NULL) - ctx->ebookdroid_file_stream_offset;
// EBD: files with garbage at beginning <<<
...
// EBD: We work only with files, So always try to load progressive >>>
//		if (fz_stream_meta(ctx, doc->file, FZ_STREAM_META_PROGRESSIVE, 0, NULL) > 0)
			doc->file_reading_linearly = (ctx->ebookdroid_linearized_load && !ctx->ebookdroid_hasPassword) ? 1 : 0;
// EBD: We work only with files, So always try to load progressive <<<
