...
// EBD: Fix for strange documents with /Auto keys instead of /Type >>>
static pdf_obj *
...
// EBD: Fix for strange documents with /Auto keys instead of /Type <<<
	...
	fz_catch(ctx)
	{
	// EBD: Ignore most errors >>>
		if (!ctx->ebookdroid_ignore_most_errors)
			fz_rethrow(ctx);
	// EBD: Ignore most errors <<<
	}
...
pdf_load_page(fz_context *ctx, pdf_document *doc, int number)
{
	...
	if (doc->file_reading_linearly)
	{
		...
		if (pageref == NULL)
		// EBD: Fix for files that only declares itself as "Linearized" >>>
		{
